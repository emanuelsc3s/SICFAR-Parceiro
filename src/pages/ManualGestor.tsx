...
const ManualGestor = () => {
  ...
  const filteredChapters = chapters.filter(chapter => chapter.title.toLowerCase().includes(searchTerm.toLowerCase()) || chapter.summary.toLowerCase().includes(searchTerm.toLowerCase()) || chapter.topics.some(topic => topic.toLowerCase().includes(searchTerm.toLowerCase())));
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <Card className="mb-8">
          <CardContent className="p-4">
            <div className="relative">
              ...
              <Input placeholder="Buscar por tópicos, capítulos ou conteúdo..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
            </div>
          </CardContent>
        </Card>
        ...
        <div className="grid gap-6">
          {filteredChapters.map((chapter, index) => <Card key={chapter.id} className="tile-card group">
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-4 flex-1">
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${chapter.completed ? 'bg-green-100 text-green-600' : 'bg-primary/10 text-primary'}`}>
                      {chapter.completed ? <CheckCircle className="h-6 w-6" /> : <chapter.icon className="h-6 w-6" />}
                    </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        {chapter.completed && <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                            Concluído
                          </Badge>}
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {chapter.topics.map((topic, topicIndex) => <Badge key={topicIndex} variant="outline" className="text-xs">
                              {topic}
                            </Badge>)}
                        </div>
                      </div>
                  ...
                  <Button variant={chapter.completed ? "default" : "outline"} className="ml-4 group-hover:scale-105 transition-transform">
